...
def main():
    ...
    LOSS_WEIGHT_L2 = 0.5
    LOSS_WEIGHT_L3 = 1.0
    SEG_LOSS_WEIGHT = 5.0 # Tăng trọng số để ưu tiên segmentation
    GRAD_CLIP_VALUE = 1.0
    ...
    # --- Hàm Visualization (đã được cập nhật) ---
    def save_visualization(model, text_projectors, text_embeddings, vis_batch, epoch, device, output_dir):
        ...
        with torch.no_grad():
            ...
            image, mask_gt, label3, valid_region_mask = image.to(device), mask_gt.to(device), label3.to(device), valid_region_mask.to(device)
            ...
            for i in range(num_images):
                ...
                else: # Trường hợp không có mask ground truth
                    idx_to_show = 0
                    class_name = 'normal'
    ...
    for epoch in range(EPOCHS):
        ...
        for step, batch_data in enumerate(pbar):
            ...
            image, mask_gt, label1, label2, label3, valid_region_mask = [d.to(device) for d in batch_data]
            ...
            with autocast():
                ...
                # --- Tính toán Segmentation Loss (Đã cập nhật) ---
            ...
            # Kiểm tra loss hợp lệ trước khi backward
